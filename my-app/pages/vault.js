...
export default function Vault() {
    ...
    // Submitting New Sneaker Form
    const onSubmit = async (data) => {
        if (uploadedFiles.length < 1) { alert("Upload at least one image"); return; }
        ...
        try {
            ...
            formData.append("quantity", data.quantity || 1); // Added Quantity
            ...
            reset();
            mutate();
        } catch (err) { alert("Failed to save sneaker"); } 
        ...
        <>
            ...
            <Container className="py-5">
                <div className="d-flex justify-content-between align-items-center mb-5">
                    ...
                    <button onClick={() => setShowModal(true)} className="btn btn-primary rounded-pill px-4 py-2 fw-bold shadow-sm">+ Add Sneaker</button>
                </div>
                ...
                <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" centered>
                    ...
                    <Modal.Body className="pt-2">
                        <Form onSubmit={handleSubmit(onSubmit)}>
                            {/* NEW: Added Quantity Row */}
                            <Row>
                                <Col md={4} className="mb-3">
                                    <Form.Label className="fw-semibold">Quantity</Form.Label>
                                    <Form.Control type="number" min="1" defaultValue="1" {...register("quantity", { required: true })} />
                                </Col>
                                <Col md={4} className="mb-3">
                            ...
                            <div className="mt-4">
                                <Form.Label className="fw-semibold">Upload Images (Max 3)</Form.Label>
                                <div 
                                    ...
                                    onClick={() => inputRef.current.click()} style={{ cursor: 'pointer', borderStyle: 'dashed' }}
                                >
                                    <input ref={inputRef} type="file" multiple accept="image/*" onChange={handleChange} className="d-none" />
                                    <p className="mb-0 fw-semibold">Drag & drop images here</p>
                                </div>
                            </div>
